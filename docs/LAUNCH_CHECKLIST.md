...
- [ ] Add **both** apex and `www` in Vercel; Vercel serves one as primary and redirects the other automatically. Make the primary match `NEXT_PUBLIC_SITE_URL` so canonical URLs agree.
- [ ] **Set that redirect to 308 Permanent** (Vercel Settings > Domains > Edit on the redirecting domain). Vercel's dashboard defaults to **307 Temporary**, which tells Google "don't consolidate these addresses yet" and leaves stale "Page with redirect" rows in Search Console. Verify from a terminal: `curl -sI https://<apexdomain>/` must show 308. (Found as a 307 on Theresa's site, fixed 2026-07-05.)
- [ ] **IndexNow key** (tells Bing/DuckDuckGo about pages instantly; Google not included): generate a key with `node -e "console.log(require('crypto').randomBytes(16).toString('hex'))"`, add it as env var `INDEXNOW_KEY` (Production) on the client's Vercel project, redeploy, then verify `https://<domain>/indexnow.txt` serves the key. After that every production deploy auto-submits the sitemap URLs (see `scripts/indexnow-submit.mjs`; build logs show `[indexnow] submitted N URLs`).
- [ ] **Enable Vercel Web Analytics + Speed Insights** on the client's project (dashboard > project > Analytics / Speed Insights > Enable). The code snippets are already in the template layout. On Hobby both are free (Analytics 50K events/mo; Speed Insights ONE project only, 10K events/mo, 7-day window). On Pro, Analytics meters $0.03 per 1K events from zero and Speed Insights bills a $10/project/mo base fee the moment you enable it: enable deliberately, not by habit.
